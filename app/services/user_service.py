...
def get_user_list(data):
    try:
        ...
        for user in result['items']:
            branch_ids = [Branch.branch_id for Branch in user.branches]
            branch_names = [Branch.branch_name for Branch in user.branches]
...
def assign_branches_to_user(data):
        ...
        if not username:
            return NotFoundError("กรุณาระบุ Username")

        if not isinstance(branch_ids, list):
            return NotFoundError("branch_ids ต้องเป็นรูปแบบ Array (List) เท่านั้น")
